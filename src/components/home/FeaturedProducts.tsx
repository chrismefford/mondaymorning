...
const FeaturedProducts = () => {
  ...
  return (
    <section id="shop" className="py-16 lg:py-40 bg-background relative overflow-hidden">
      {/* Background number */}
      <div className="absolute top-0 right-0 font-serif text-[12rem] lg:text-[40rem] font-bold text-muted/20 leading-none pointer-events-none select-none -translate-y-1/4 translate-x-1/4">
        01
      </div>
      ...
      <div className="relative z-10">
        ...
        <div className="container mx-auto px-4 lg:px-8 mb-8 lg:mb-24">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 lg:gap-6">
            <div>
              <span className="font-sans text-[10px] lg:text-xs font-semibold uppercase tracking-[0.3em] text-primary mb-2 lg:mb-4 block">
                ( Featured )
              </span>
            ...
            <Button 
              variant="ghost" 
              className="hidden lg:flex font-sans text-sm font-semibold uppercase tracking-wider group self-start lg:self-auto border-2 border-transparent hover:border-foreground px-6 py-3"
            >
              View all
        ...
        <div className="lg:hidden mb-12">
            ...
            <div className="flex-shrink-0 w-[85vw] snap-center">
              <div className="relative bg-card border-2 border-foreground overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 text-background">
                  <span className="font-sans text-[10px] uppercase tracking-[0.3em] text-background/60">Bestseller</span>
                  <h3 className="font-serif text-2xl font-bold mt-1">{featuredProduct.name}</h3>
                  <p className="font-sans text-sm text-background/70 mt-2 line-clamp-2">{featuredProduct.description}</p>
                  <div className="flex items-center justify-between mt-4">
                    <span className="font-sans text-xl font-bold">${featuredProduct.price}</span>
                    <Button size="sm" className="bg-primary text-primary-foreground font-sans text-xs uppercase tracking-wider">
                      Add to Cart
                    </Button>
                  </div>
                </div>
          ...
          <div className="px-4 mt-8">
            <Button 
              variant="outline"
              className="w-full font-sans text-sm font-bold uppercase tracking-widest py-5 border-2 border-foreground"
            >
              View All Products
